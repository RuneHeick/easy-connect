...
uint8 GAPManget_SetupName()
{
    ...
    deviceName_GetName(&scanRspData[size],buffersize,2);
    size = size + nameSize;
...
{
  // Flags; this sets the device to use general discoverable
  0x02,   // length of this data
  GAP_ADTYPE_FLAGS,
...
void GAPManeger_Setup()
{
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    uint8 scanRspSize = GAPManget_SetupName();
    
    // Set the GAP Role Parameters
    GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &initial_advertising_enable );
    GAPRole_SetParameter( GAPROLE_ADVERT_OFF_TIME, sizeof( uint16 ), &gapRole_AdvertOffTime );

    GAPRole_SetParameter( GAPROLE_SCAN_RSP_DATA, scanRspSize , scanRspData );
