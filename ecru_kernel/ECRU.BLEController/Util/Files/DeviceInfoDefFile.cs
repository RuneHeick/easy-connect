...
namespace ECRU.BLEController.Util
{
    internal class DeviceInfoDefFile
    {
        ...
        {
            var ByteArray = new byte[GetSize(deviceInfo)];
            int index = 0;
            ...
            ByteArray[index++] =
                (byte) (2 + (deviceInfo.Manufacture.Value != null ? deviceInfo.Manufacture.Value.Length : 0));
            ...
            foreach (Service s in deviceInfo.Services)
            {
                ...
                foreach (Characteristic c in s.Characteristics)
                {
                    ...
                    ByteArray[index++] = c.Value.ReadWriteProps;
            ...
            try
            {
                ...
                while (i < data.Length)
                {
                    ...
                    switch (Command)
                    {
                        ...
                        case 0xC0:
                            charIndex++;
                            file.Services[serviceIndex].Characteristics[charIndex].Value.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].Value.ReadWriteProps = data[i + 4];
                            break;
                        case 0xC1:
                            file.Services[serviceIndex].Characteristics[charIndex].Description.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].Description.Value =
                                data.GetPart(i + 4, len - 2);
                            break;
                        case 0xC2:
                            file.Services[serviceIndex].Characteristics[charIndex].Format.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].Format.Value = data.GetPart(i + 4,
                                len - 2);
                            break;
                        case 0xC3:
                            file.Services[serviceIndex].Characteristics[charIndex].GUIFormat.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].GUIFormat.Value = data.GetPart(
                                i + 4, len - 2);
                            break;
                        case 0xC4:
                            file.Services[serviceIndex].Characteristics[charIndex].Range.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].Range.Value = data.GetPart(i + 4,
                                len - 2);
                            break;
                        case 0xC5:
                            file.Services[serviceIndex].Characteristics[charIndex].Subscription.handle =
                                (UInt16) ((data[i + 2] << 8) + data[i + 3]);
                            file.Services[serviceIndex].Characteristics[charIndex].Subscription.Value =
                                data.GetPart(i + 4, len - 2);
                            break;
                    }
        ...
        private static void MakeDeviceInfoReady(byte[] data, DeviceInfo file)
        {
            ...
            {
                var starterInfo = items[z] as ServiceStarter;
