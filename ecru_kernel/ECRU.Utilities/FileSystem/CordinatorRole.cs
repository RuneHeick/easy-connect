...
namespace ECRU.Utilities
{
    ...
    {
        private const int MUTEX_MAX_LOCKTIME = 300000; // 5 min
        private const string CordinatorType = "CordinatorRq";
        private const int ConnectionTimeOut = 10000;

        private readonly LocalManager InfoManager;
        private readonly object Lock = new object();

        private readonly Thread MutexHandler;
        ...
        private CordinatorState state { get; set; }
        ...
        public void Dispose()
        {
            ...
            {
                var mutex = Mutexs[0] as FileMutex;
                if (mutex != null)
                {
                ...
                if (msg.connectionType == CordinatorType)
                {
                    ...
                    if (con != null)
                    {
                        ...
                        while ((DateTime.Now - StartTime).Milliseconds < ConnectionTimeOut)
                        {
                            try
                            {
                                if ((!con.Poll(10, SelectMode.SelectRead) && !con.Poll(10, SelectMode.SelectError)) ==
                                    false)
                                    break;

                                if (con.Available > 0)
                                {
                                    var buffer = new byte[con.Available];
                                    con.Receive(buffer);
                                    ...
                                    switch (buffer[0])
                                    {
                                        ...
                                        case 0x04: // Rqu File 
                                        {
                                            ...
                                            if (file != null && info != null)
                                            {
                                                ...
                                                var data = new byte[2 + fileLength + 2 + infoLength];
        ...
        private void HandelMutexConnections()
        {
            try
            {
                while (Disposed == false)
                {
                    ...
                    {
                        var mutex = Mutexs[i] as FileMutex;
                        if (mutex != null)
                        {
        ...
        {
            var UsersToUpdate = new ArrayList();
            foreach (string RU in Users)
            {
                if (RU != SystemInfo.SystemMAC.ToHex())
            ...
            foreach (string RU in Users)
            {
                if (RU != SystemInfo.SystemMAC.ToHex())
                {
                    var rq = new NewConnectionMessage
                    {
                        Receiver = RU.FromHex(),
                        ConnectionType = NetworkManager.connectionRQType,
                        ConnectionCallback =
                            (socket, r) => UpdateFileConnection(mutex.path, r, socket, UsersToUpdate, this)
                    };
                    EventBus.Publish(rq);
                }
            }
        ...
        private static void UpdateFileConnection(string path, byte[] receiver, Socket socket, ArrayList UsersToUpdate,
            CordinatorRole item)
        {
            if (item.Disposed)
            {
            ...
            if (socket != null)
            {
                ...
                try
                {
                    ...
                    if (file != null)
                    {
                        ...
                        var data = new byte[1 + + 2 + pathlen + 2 + fileLength + 2 + infoLength];
                        data[0] = 0x03;
            ...
            else
            {
                var rq = new NewConnectionMessage
                {
                    Receiver = receiver,
                    ConnectionType = NetworkManager.connectionRQType,
                    ConnectionCallback = (s, r) => UpdateFileConnection(path, r, s, UsersToUpdate, item)
                };
                EventBus.Publish(rq);
            }
        }
        ...
        {
            var PacketSendTo = new ArrayList();
            var FileState = new Hashtable();
            ...
            foreach (FileInfo f in files)
            {
                ...
                if (b != null)
                {
                    ...
                    AddINTable(filename, FileState, infof.Version, SystemInfo.SystemMAC, this);
                }
            }
            ...
            foreach (string RU in Users)
            {
                if (RU != SystemInfo.SystemMAC.ToHex())
                {
                    var rq = new NewConnectionMessage
                    {
                        ...
                        ConnectionType = NetworkManager.connectionRQType,
                        ConnectionCallback =
                            (socket, resiver) => FileStateConnection(socket, resiver, PacketSendTo, FileState, this)
                    };
                    EventBus.Publish(rq);
                }
            }
        ...
        private static void FileStateConnection(Socket socket, byte[] resiver, ArrayList PacketSendTo,
            Hashtable FileState, CordinatorRole item)
        {
            if (item.Disposed)
            {
                if (socket != null)
            ...
            if (socket != null)
            {
                try
                {
                    ...
                    {
                        waitingForData = !socket.Poll(-1, SelectMode.SelectRead) &&
            ...
            else //Retry;
            {
                var rq = new NewConnectionMessage
                {
                    Receiver = resiver,
                    ConnectionType = NetworkManager.connectionRQType,
                    ConnectionCallback = (so, re) => FileStateConnection(so, re, PacketSendTo, FileState, item)
                };
                EventBus.Publish(rq);
            }
        }

        private static void AddINTable(string Path, Hashtable FileState, long version, byte[] resiver,
            CordinatorRole item)
        {
            if (item.Disposed == false)
            {
                ...
                {
                    var file = FileState[Path] as Hashtable;
        ...
        private void GetNewestFiles(Hashtable FileState)
        {
            ...
            lock (FileState)
            {
                ...
                {
                    var Filestates = FileState[key] as Hashtable;
                    ...
                    if (currentFile == null || currentFile.Version < best.version)
                    {
                        var rq = new NewConnectionMessage
                        {
                            ...
                            ConnectionCallback =
                                (socket, resiver) =>
                                    GetFileConnection(socket, resiver, path, FileState, best.version, this)
                        };
                        EventBus.Publish(rq);
                        HasAllFiles = false;
                    }
                    else
                    {
                        lock (FileState)
                        {
                            Filestates.Remove(SystemInfo.SystemMAC.ToHex());
                        }
        ...
        private static void GetFileConnection(Socket socket, byte[] resiver, string path, Hashtable MasterFileStates,
            long version, CordinatorRole item)
        {
            if (item.Disposed)
            {
                if (socket != null)
            ...
            else
            {
                var rq = new NewConnectionMessage
                {
                    Receiver = resiver,
                    ConnectionType = NetworkManager.connectionRQType,
                    ConnectionCallback = (so, re) => GetFileConnection(so, re, path, MasterFileStates, version, item)
                };
                EventBus.Publish(rq);
            }
        }
        ...
        private static void UpdateFiles(string path, Hashtable MasterFileStates, long version, CordinatorRole item)
        {
            lock (MasterFileStates)
            {
                ...
                {
                    var FileStates = MasterFileStates[path] as Hashtable;
                    FileStates.Remove(SystemInfo.SystemMAC.ToHex()); // Remove Self. 
                    ...
                    foreach (object key in FileStates.Keys)
                    {
                        ...
                        if (ver < version)
                        {
                            var rq = new NewConnectionMessage
                            {
                                Receiver = mac.FromHex(),
                                ConnectionType = NetworkManager.connectionRQType,
                                ConnectionCallback = (s, r) => UpdateFilesAtStartup(s, r, path, MasterFileStates, item)
                            };
                            EventBus.Publish(rq);
                        }
                        else
        ...
        private static void UpdateFilesAtStartup(Socket socket, byte[] resiver, string path, Hashtable MasterFileStates,
            CordinatorRole item)
        {
            if (item.Disposed)
            {
